...
class MySQLDB extends DB {
	...
	public function connect($host, $port, $usuario, $senha, $banco) {
		try {
            $pdo = new PDO('mysql:host='.$host.';port='.$port.';dbname='.$banco.'', $usuario, $senha, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
	...
	public function inserir_cargo($id_cargo_pai, $nm_cargo, $desc_cargo) {
		...
		$sql = "Select MAX(id_cargo) FROM cargos_aninhados";
		$statement = $this->get_connection()->prepare($sql);
		$statement->execute();
		$resul = $statement->fetch(PDO::FETCH_NUM); 
        $id_cargo = $resul[0];
	...
	public function criar_grupo($id_grupo_pai, $nm_grupo, $desc_grupo) {
		...
		$sql = "Select MAX(id_grupo) FROM grupos_aninhados";
		$statement = $this->get_connection()->prepare($sql);
		$statement->execute();
		$resul = $statement->fetch(PDO::FETCH_NUM); 
        $id_grupo = $resul[0];
		
		//$id_grupo = $this->get_connection()->lastInsertId();
	...
	public function inserir_usuario($login_usu, $nm_usu, $ult_nm_usu, $senha_usu) {
		...
		$sql = "Select MAX(id_usu) FROM usuarios";
		$statement = $this->get_connection()->prepare($sql);
		$statement->execute();
		$resul = $statement->fetch(PDO::FETCH_NUM); 
        $id = $resul[0];
		
		//$id = $this->get_connection()->lastInsertId();
