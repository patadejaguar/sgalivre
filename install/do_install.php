    ...
    try {
        ...
        try {
            ...
            if ($db['db_type'] == 'mysql') {
                ...
                $dbcon = @mysql_connect('' . $db['db_host'] . ':' . $db['db_port'] . '', '' . $db['db_user'] . '', '' . $db['db_pass'] . '');

                if ($dbcon === FALSE) {
                    throw new Exception(mysql_last_error($dbcon));
                }

                $result = @mysql_query('DROP DATABASE IF EXISTS ' . $db['db_name']);
                ...
                $result = @mysql_query('CREATE DATABASE ' . $db['db_name'] . " DEFAULT CHARACTER SET 'UTF8' COLLATE 'utf8_general_ci'; ");
                if (!$result) {
                    throw new Exception(mysql_last_error($dbcon));
                }
                ...
                $dbcon = @mysql_connect('' . $db['db_host'] . ':' . $db['db_port'] . '', '' . $db['db_user'] . '', '' . $db['db_pass'] . '', '' . $db['db_name'] . '');
                ...
                foreach ($adm as $key => $value) {
                    $escaped[$key] = mysql_escape_string($value);
                }
                ...
                //cria um array para cada query

                $array_sql = explode(";--", $sql);
                foreach ($array_sql as $query) {
                    mysql_select_db('' . $db['db_name'] . '', $dbcon);
            ...
            if ($db['db_type'] == 'pgsql') {
                ...
                if (!$result) {
                    throw new Exception(pg_last_error($dbcon)."sga");
                }
                pg_close($dbcon);
                ...
                if (!$result) {
                    throw new Exception(pg_last_error($dbcon));
                    /*
                    $sql = file_get_contents('install/sql/' . $db_type . '_install2.sql');
                    $db_class = 'PgSQLDB';
